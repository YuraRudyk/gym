(function() {
   ...
   */
  var formAuthentication = function(form) {
    var self = this;
    ...
    this.handle = function() {
      ...
      self.xhr.onreadystatechange = function() {
        ...
        if (self.xhr.readyState === 4 && self.xhr.status > 0) {
          handleResponse();
        }
      };

      self.xhr.open('POST', self.form.getAttribute('action'), true);
      self.xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
      self.xhr.send(encodeUrlData(self.form));
    };

    /**
     * Initialize XMLHttpRequest object
     */
    var initXhrObject = function() {
      if (self.xhr) {
        self.xhr.abort();
      }
     ...
     * @returns {string}
     */
    var encodeUrlData = function(form) {
      var encodedParts = [];
      ...
      for (var i = form.elements.length; i--;) {
        ...
        if (field.nodeName.toLowerCase() === 'input') {
          encodedParts.push(encodeURIComponent(field.name) + '=' + encodeURIComponent(field.value));
        }
      }

      return encodedParts.join('&').replace(/%20/g, '+');
    };

    /**
     * Handle the ready state of XHR Object
     */
    var handleResponse = function() {
      var data   = JSON.parse(self.xhr.responseText),
          errors = document.getElementById('login-errors');

      errors.innerHTML = '';

      if (data.success && !data.hasOwnProperty('error')
        && data.hasOwnProperty('redirectUrl') && data.redirectUrl.length) {
  ...
  var rsaEncryption = function(form) {
    ...
    this.handleFormSubmitRequest = function(event) {
      ...
      if (rsaEncryption.fields.length) {
    ...
    this.handlePublicKeyResponse = function(response, rsaEncryption) {
      ...
      (new formAuthentication(rsaEncryption.form)).handle();
    };
  };
