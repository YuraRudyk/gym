;(function ($, window) {
    ...
    var vimelar = function(node, options) { // should be called on the wrapper div
        ...
        $('<iframe />', {
            ...
            allowfullscreen: 1
        }).prependTo('#video-section').wrap('<div id="vimelar-container" style="overflow: hidden; position: absolute; z-index: 1; width: 100%; height: 100%; top: 50%; left: 50%; transform: translate(-50%,-50%)"></div>').after('<div id="vimelar-overlay" style="width: 100%; height: 100%; z-index: 2; position: absolute; left: 0; top: 0;"></div>');
        ...
        var resize = function() {
            ...
            if (width / options.ratio < height) { // if new video height < window height (gap underneath)
                ...
                $vimelarPlayer.width(pWidth).height(height); // player width is greater, offset left; reset top
            } else { // new video width < window width (gap to right)
                pHeight = Math.ceil(width / options.ratio); // get new player height
                $vimelarPlayer.width(width).height(pHeight); // player height is greater, offset top; reset left
            }
